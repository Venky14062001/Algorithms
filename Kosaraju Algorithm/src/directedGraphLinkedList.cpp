...
template <typename T>
void Graph<T>::reverseGraph()
{
    //Iterate through the node and add connections to the reverse_node_connections vector
    for (auto i=graph.begin(); i!=graph.end(); ++i)
    {
        for (auto j: (*i)->node_connections)
        {
            (j->reverse_node_connections).push_back(*i);
        }
    }
}

template <typename T>
void Graph<T>::printReverseGraph()
{
    for (auto i=graph.begin(); i!=graph.end(); ++i)
    {
        std::cout<<"Printing the connections for vertex "<<((*i)->node_name)<<'\n';
        ...
        for (Node<T>* j: (*i)->reverse_node_connections)
        {
            std::cout<<"--->"<<j->node_name;
        }
